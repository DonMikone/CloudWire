...
| **Offline Items** | Any folders and files of a Connection, picked in a tree with checkboxes and mirrored at their cloud path below the Storage Location, kept as real local files and synced both ways with rclone bisync. The selection can be changed later. Choose the Storage Location per item, including external drives; moving it relocates files without downloading again. Conflicts keep both versions (`Bassline.conflict 2026-09-23 1405.wav`). A Mass-Delete Guard stops any run that would delete more than half of an item until you decide. |
...
3. **Make folders offline.** Right-click folders or files inside the Mount and choose **Offline verfügbar machen** (Make available offline), or use **Offline** in the app and check any folders and files of a Connection in the tree. By default they land at their cloud path below `~/CloudWire/<Connection>`; click **Ändern...** to choose another folder, for example on an external SSD, which then holds the items at their cloud path. CloudWire checks the free space first and then downloads everything once. **Auswahl ändern...** (Change Selection) adds or removes items later.
