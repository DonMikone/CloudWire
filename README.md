...
2. **Add a Mount.** In **Laufwerke** (Mounts), pick the connection and a cloud folder. The drive appears in Finder under `~/CloudWire/Laufwerke/<Name>` and mounts again automatically at login.
3. **Make a folder offline.** Right-click a folder inside the Mount and choose **Offline verfügbar machen** (Make available offline), or use **Offline** in the app. By default the files land in `~/CloudWire/<Connection>/<path>`; click **Ändern...** to put them anywhere, for example on an external SSD. CloudWire checks the free space first and then downloads everything once.
